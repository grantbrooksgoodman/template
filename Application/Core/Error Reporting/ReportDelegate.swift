...
public class ReportDelegate: UIViewController, AKReportDelegate, MFMailComposeViewControllerDelegate {
    ...
    public func fileReport(error: AKError) {
        ...
        let logFile = LogFile(fileName: "\(Build.codeName.lowercased())_\(dateHashlet)",
                              directoryName: "",
                              data: data)
    ...
    public func fileReport(forBug: Bool,
        ...
        let logFile: LogFile = .init(fileName: "\(Build.codeName.lowercased())_\(dateHashlet)",
                                     directoryName: "",
                                     data: logFileData)
        ...
        translator.getTranslations(for: [.init(body),
                                         .init(prompt)],
                                   languagePair: .init(from: "en", to: RuntimeStorage.languageCode!)) { translations, errorDescriptors in
    ...
    private func getLogFileData(type: ContextCodeType,
                                error: AKError? = nil,
        ...
        guard let contextCode = akCore.contextCode(for: type, metadata: error?.metadata ?? metadata!) else {
            Logger.log("Unable to generate context code.",
                       with: .fatalAlert,
                       metadata: [#file, #function, #line])
            return nil
        }
    ...
    private func composeMessage(_ messageBody: (message: String, isHTML: Bool)? = nil,
        ...
        self.core.ui.present(composeController)
    }
    ...
    public func mailComposeController(_ controller: MFMailComposeViewController,
        ...
        controller.dismiss(animated: true) {
            self.core.gcd.after(seconds: 1) {
                if result == .failed {
                    AKErrorAlert(error: Exception(error!, metadata: [#file, #function, #line]).asAkError(),
